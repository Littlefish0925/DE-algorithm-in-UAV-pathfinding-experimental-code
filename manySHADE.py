...
def check_interpath_collision(path1, path2, num_samples=10):
    for i in range(len(path1) - 1):
        for j in range(len(path2) - 1):
            if check_collision(path1[i], path1[i + 1], (path2[j], path2[j + 1]), num_samples=num_samples):
                return True
    return False
...
obstacles = [
    (np.array([2, 2, 0]), np.array([3, 4, 9])),
    (np.array([4, 4, 0]), np.array([5, 5, 9])),
    (np.array([6, 6, 0]), np.array([7, 7, 9])),
    (np.array([8, 8, 0]), np.array([9, 9, 9])),
