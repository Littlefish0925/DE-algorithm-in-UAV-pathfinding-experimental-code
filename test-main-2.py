...
def is_collision(point, obstacle):
    if obstacle["type"] == "cuboid":
        ...
        return np.linalg.norm(point[:2] - center[:2]) <= radius and 0 <= point[2] <= height
...
def check_collision(point1, point2, obstacle, num_samples=10):
    ...
    for point in points:
        if obstacle["type"] == "cuboid":
            ...
            radius = obstacle["radius"]
            height = obstacle["height"]
            if np.linalg.norm(point[:2] - center[:2]) <= radius and 0 <= point[2] <= height:
                return True
    return False
...
# Define multiple obstacles including cuboids and cylinders
obstacles = [
...
# Run SHADE algorithm
start_time = time.time()
best_path_shade, shade_collisions = shade(objective_function, bounds, max_gen=200)
...
ax.plot(best_path_shade[:, 0], best_path_shade[:, 1], best_path_shade[:, 2], 'go-', label='SHADE Path')
...
plt.title('3D Path Planning: A* vs SHADE vs GA vs PSO')
plt.show()
